(function(root) {
    ...
    */
	...
	//使用模板标识符<% %>  evalute  js逻辑捕获    interpolate  变量捕获   escape 逃逸的字符捕获  模式1| 模式2
	//以默认配置为优先 以用户配置为覆盖
	...
	_.template = function(templateString, settings){
		...
		templateString.replace(matcher, function(match, interpolate, escape, evalute,  offset){
			//offset
	         source += templateString.slice(index,offset).replace(escapeExp, escapeChar);   // "hello: "
