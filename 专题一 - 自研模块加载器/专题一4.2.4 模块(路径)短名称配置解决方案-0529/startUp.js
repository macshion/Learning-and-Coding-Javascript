(function(global) {
  ...
  startUp.resolveMain = function(id, uri) {
	...
	Module.prototype.exec = function() {
      ...
      // console.log(startUp.resolveMain(id, uri))
			return startUp.resolveMain(id, uri);
		}
    ...
    console.log(ids)
      ...
      uris[i] = startUp.resolveMain(ids[i], mod.uri); //依赖项   (主干| 子树)
    }
		return uris;
  }
    ...
    var module = Module.get(uri, isArray(deps) ? deps : [deps]);
      ...
      var uris = module.resolve()//module.resolve();
      // console.log(uris)
    ...
    //判断是否配置data-main属性
    var scriptPath = getDataMainPath(global);
    var path = data.cwd + "_use_" + cid();
    if(scriptPath){
      path = data.cwd + scriptPath
      // console.log(path)
    }
    // console.log(path)
		//检测有没有预先加载的模块  
		Module.preload(function() {
			Module.use(list, callback, path); //虚拟的根目录
		});
	}
  ...
  // 获取当前script标签的data-main属性
  function getDataMainPath(window) {
      var scripts = window.document.getElementsByTagName("script");
      var dataMain;
      for(var i=0;i<scripts.length;i++){
        if(scripts[i].dataset.main){
          dataMain = scripts[i].dataset.main
        }
      }
      return dataMain;
  }
