(function(global) {
	...
	Module.prototype.exec = function() {
		...
		function require(id) {
			...
			// console.log(startUp.resolve(id, uri));
		...
		return exports; //module上扩展的exports
	}
	...
	Module.get = function(uri, deps) {
		//cache.xx.js = {} //Module实例对象
		return cache[uri] || (cache[uri] = new Module(uri, deps));
	}

	Module.use = function(deps, callback, uri) {
		var module = Module.get(uri, isArray(deps) ? deps : [deps]); //Module实例对象
		//所有模块都加载完毕
		module.callback = function() {
			var exports = []; //所以依赖项模块的接口对象
			var uris = module.resolve(); //获取依赖项对应的绝对路径地址
