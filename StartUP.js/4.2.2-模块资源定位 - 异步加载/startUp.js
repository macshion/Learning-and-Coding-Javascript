(function(global) {
	...
	function addBase(id, uri) {
		var result;
		if (id.charAt(0) === ".") { //相对路径 id 当前模块名称 eg：“a.js”
	...
	startUp.resolve = function(child, parent) {
		...
		return addBase(child, parent); //添加根目录 parent当前项目绝对路径
	}
	...
	Module.prototype.load = function() {
		var module = this;
		module.status = status.LOADING;//==3 正在加载依赖项
		...
		for (var i = 0; i < len; i++) {
			...
			if (m.status < status.LOADED) { //LOADED => 4
		...
		for (var i = 0; i < len; i++) {
			m = Module.get(uris[i]);
			if (m.status < status.FETCHED) { //FETCHED == 1  正在获取当前模块uri
