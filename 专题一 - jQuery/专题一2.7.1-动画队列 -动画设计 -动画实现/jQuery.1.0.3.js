...
(function(root) {
	...
	Data.prototype = {
		...
		set: function(owner, data, value) {
			var prop,
				...
				cache = this.cache[unlock]; //创建当前owner元素 缓存数据对象
	...
	jQuery.extend({
		...
		dequeue: function(elem, type) {
			type = type || "fx";
			var queue = jQuery.queue(elem, type);     //缓存中队列的信息   maxqueue 信息 [处理函数,处理函数,处理函数]
	...
	jQuery.fn.extend({
		queue: function(type, data) {
			...
			return data === undefined ?
				...
				//每一个DOM元素
				this.each(function() {
					var queue = jQuery.queue(this, type, data);   //队列的信息
					if (type === "fx" && queue[0] !== "inprogress") {
						jQuery.dequeue(this, type);   //出列 
					}
				});
		},
		...
		animate: function(options) {  //left:50     duration 2000 
			...
			//logic  主要逻辑
			function logic() {
				//  动画开始的时间(固定) +  整体动画运行的时间(固定)   -   执行logic获取的时间(增长)
				//返回一个小于2000> 数值  第一次 remaining      0
				var remaining = Math.max(0, startTime + options.duration - createTime())
				//console.log(remaining);  无限接近2000  ->  0
				var temp = remaining / options.duration || 0;   //   remaining  /  2000
				//console.log(temp);   // 无限接近1  ->  0
				var percent = 1 - temp;   //无限接近0 - > 1

				var setStyle = function(value) {
					elem[i].style['left'] = value + 'px';    // 硬编码
				}
				//移动的距离   -450*percent     无限接近0 - > 1    nowMove  无限接近500  -> 50
